...
db.persistence.setAutocompactionInterval(60000); //compact db every 60sec
...
app.post('/register', function(req, res) {
  ...
  if(!req.body) {
    res.status(400);
    return res.send(JSON.stringify({status: "error", message: "No body sent"}));
  }
  ...
  if(!req.body.webhook_type) {
    ...
    if(!['signer_created', 'signer_removed', 'signer_updated',
          'account_created', 'account_removed', 'account_debited',
          'account_credited'].includes(req.body.webhook_type)) {
      res.status(400);
      return res.send(JSON.stringify({status: "error", message: "Incompatible webhook type"}));
    }
    ...
    if(!req.body.address_id || !req.body.webhook_url) {
      ...
      if(!validate.isURL(req.body.webhook_url)) {
        return res.send(JSON.stringify({status: "error", message: "Webhook URL invalid"}));
      }
      if(!validate.isAddress(req.body.address_id)) {
        return res.send(JSON.stringify({status: "error", message: "Stellar address invalid"}));
      }
    }
      ...
      if(!req.body.asset_code) {
        ...
        if(req.body.asset_code !== 'XLM') {
          if(!req.body.asset_address) {
            ...
            return res.send(JSON.stringify({status: "error", message: "Asset issuer invalid"}));
          }
        }

        if(!validate.isAsset(req.body.asset_code)) {
          return res.send(JSON.stringify({status: "error", message: "Asset code invalid"}));
        }
      }
    }
  }
  ...
  if(badParamErr) {
    res.status(400);
    return res.send(JSON.stringify({status: "error", message: "Incorrect parameters sent"}));
  }
  ...
  //TODO asset_code and issuer should be an array of objects?
  db.findOne({
    ...
    if(!err) {
      if(!doc) {
        db.update({
          ...
          if(!err) {
            return res.send(JSON.stringify({status: "success", message: "Webhook successfully added"}));
          } else {
        ...
        return res.send(JSON.stringify({status: "warning", message: "Webhook already exists"}));
      }
    } else {
